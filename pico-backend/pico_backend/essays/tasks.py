...
from essays.constants import (
    ...
    TEXTRACT_PROFILE_NAME,
    TEXTRACT_REGION_NAME,
)
...
TINIFY_API_KEY = settings.TINIFY_API_KEY
...
def get_textractor():
    ...
    if settings.DEBUG:
        return Textractor(
            profile_name=TEXTRACT_PROFILE_NAME,
            region_name=TEXTRACT_REGION_NAME,
        )
    else:
        return Textractor(region_name=TEXTRACT_REGION_NAME)
...
def mock_call_tinify(essay: Essay) -> bytes:
    logger.debug(f"Mocked call to tinify for essay {essay.id}")
    return essay.original_file.file.read()


def call_tinify(essay: Essay) -> bytes:
    logger.debug(
        f"Calling tinify for essay {essay.id} with url {essay.original_file.url}"
    )
    source = tinify.from_url(essay.original_file.url)
