...
NUMBER_OF_EMBEDDING_DIMENSIONS = 1024  # a bit less than 8191
MAX_TOKENS_FOR_EMBEDDING_MODEL = 8100  # less than the max to give a buffer
...
client = OpenAI(
    api_key=API_KEY, timeout=DEFAULT_TIMEOUT, max_retries=DEFAULT_MAX_RETRIES
)
...
def get_completion_parsed(
    ...
    if model in ["o3-mini", "o4-mini"]:
        if reasoning_effort is None:
            raise ValueError("reasoning models must have reasoning_effort")
        if temperature is not None:
            logger.warning("temperature is ignored for reasoning models")

        return _openai_request(
            endpoint="chat_completion_parsed",
            model=cast(Literal["o3-mini", "o4-mini"], model),
        ...
        if temperature is None:
            raise ValueError(
                "temperature is required for models other than reasoning models"
            )
...
async def delatexify(text: str) -> str:
    """
    Converts LaTeX expressions in the input text to plain text.
    """
    logger.info("Converting LaTeX to plain text with model=%s", DELATEXIFY_MODEL)
    response = await _aopenai_request(
        endpoint="chat_completion",
        model=DELATEXIFY_MODEL,
        temperature=0,
        messages=[
            {"role": "system", "content": SYSTEM_MESSAGE_DELATEXIFY},
    ...
    logger.debug("Successfully converted LaTeX to plain text")
    return response.content
...
def validate_embedding_input(input_item: str):
...
def _openai_request(
    *,
    ...
    model: str,  # Qualquer modelo exceto "o3-mini", "o4-mini"
    temperature: float,
    messages: list[ChatCompletionMessageParam],
    response_format: type[ModelT],
    json_mode: bool = False,
...
def _openai_request(
    *,
    endpoint: Literal["chat_completion_parsed"],
    model: Literal["o3-mini", "o4-mini"],
    temperature: NotGiven = NOT_GIVEN,  # modelos de reasoning não aceitam temperature
    messages: list[ChatCompletionMessageParam],
    response_format: type[ModelT],
...
def _openai_request(
    *,
    ...
    if endpoint == "chat_completion":
        ...
        # Handle the special case for o3-mini, o4-mini with reasoning_effort
        if model in ["o3-mini", "o4-mini"] and reasoning_effort is not None:
            # Para reasoning models, não passamos temperature
...
async def _aopenai_request(
    *,
    endpoint: Literal["chat_completion_parsed"],
    model: str,  # Qualquer modelo exceto reasoning models
    temperature: float,
    messages: list[ChatCompletionMessageParam],
    response_format: type[ModelT],
    json_mode: Literal[False] = False,
...
def mock_openai_request(
    *,
    ...
    if endpoint == "chat_completion":
        ...
        def get_mock_value(field_info) -> Any:
        ...
        def mock_nested_model(model_class):
            fields_dict = {}
        ...
        if isinstance(input_text, str):
            ...
            raise ValueError(
                "input_text must be a string or list of strings for embeddings endpoint."
            )
    else:
        raise ValueError(f"Unsupported endpoint: {endpoint}")
