...
def migrate_quiz_to_session(apps, schema_editor):
    ...
    # Keep track of order by quiz_id
    order_counters = {}

    for quiz_question in quiz_questions_iterator:
        # Use the order based on the sequence of questions by ID
        quiz_id = quiz_question.quiz_id
        if quiz_id not in order_counters:
            order_counters[quiz_id] = 0

        order = order_counters[quiz_id]
        order_counters[quiz_id] += 1
...
def reverse_migrate_quiz_to_session(apps, schema_editor):
    ...
    if not session_questions.exists():
        ...
        quiz_questions_to_create = [
            QuizQuestion(quiz_id=sq["session_id"], question_id=sq["question_id"])
