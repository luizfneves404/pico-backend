...
from quiz.models import ENEM_AREAS, Question
...
As mensagens enviadas terão:
    ...
    - Quatro ou cinco alternativas, com indicações se são corretas ou incorretas
Com base nessas informações, você deve elaborar uma explicação concisa de por que a alternativa correta está correta e por que as demais estão erradas. Comente cada uma das alternativas incorretas e aponte o erro nelas."""
...
Lembre-se: se houver qualquer discrepância entre a resposta destacada das alternativas e a conclusão da resolução, ou se a justificativa for incorreta ou insuficiente, responda 'Não'. Devolva a resposta no campo fornecido."""
...
SYSTEM_MESSAGE_FIND_ERRORS = """Você é um assistente que avalia se uma questão de vestibular está completa e correta em termos de formato e dados fornecidos. 
Analise a questão (com texto, imagens, links ou equações) e retorne APENAS um número de 0 a 100 que indica a probabilidade de a questão estar completa/correta.

Use uma avaliação bastante rigorosa. Se notar qualquer problema sério, reduza significativamente a probabilidade (podendo chegar a 0%).

Exemplos de erros que devem reduzir a probabilidade:
- Imagem faltando: O enunciado diz "observe o gráfico/figura", mas o gráfico/figura não está disponível
- Imagem incorreta: O enunciado descreve algo diferente do que aparece na imagem
- Falta de texto fundamental: O enunciado faz referência a tabelas ou texto complementar que não foi fornecido
- Equações corrompidas ou ilegíveis: Trechos em LaTeX quebrados ou sem sentido
- Dados numéricos faltando ou inconsistentes: A questão pede para calcular algo usando valores não informados
- Texto truncado ou incompleto: A questão fica sem enunciado claro
- Qualquer outra inconsistência que impeça uma resposta coerente

Escala de probabilidade (exemplos de referência):
90–100%: Questão completa, sem sinais de erro ou falta de dados.
70–89%: Questão praticamente completa, mas você notou algum detalhe menor ou possível ambiguidade.
40–69%: Questão possivelmente incompleta; há algum problema, mas ainda há materiais quase suficientes para responder.
10–39%: Questão bastante incompleta ou inconsistente; a maior parte dos dados está ausente ou corrompida.
0–9%: Questão totalmente inviável de responder; faltam elementos cruciais ou quase tudo está errado.

Retorne APENAS o número, sem texto adicional, explicações ou o símbolo %.
"""
...
CATEGORIES_WITH_SUBCATEGORIES = {
    ...
    "Matemática": [
        ...
        "Funções (Função Afim, Função Exponencial, Função Quadrática, Logaritmo, Função Trigonométrica, Função Modular)",
        "Séries (Progressão Aritmética, Progressão Geométrica, Sequência Numérica)",
        "Probabilidade (Evento Único, Eventos Condicionais, Probabilidade Complementar)",
    ...
    "Física": [
        ...
        "Óptica (Fenômenos Ópticos, Lentes e Instrumentos)",
    ...
    "Química": [
        ...
        "Cálculo Químico (Estequiometria, Soluções e Concentração, Equilíbrio Químico, Termoquímica)",
...
# Gambiarra to avoid wasting hours adding the response_effort parameter to openai_utils -> I'm in a rush
...
def categorize_question(self, question_id: int, temperature: float = 0):
    try:
        ...
        if not question.is_active:
            logger.info(f"Ignorando questão inativa: {question_id}")
            return
        ...
        if (
            question.subject not in ["Matemática", "Química", "Física"]
            and question.category
        ):
            logger.info(f"Ignorando questão de {question.subject}: {question_id}")
            return
...
def generate_answer(self, question_id: int, temperature: float = 0):
    """
    Tarefa principal para gerar e validar a resposta de uma questão.
    """
    try:
        question = Question.objects.prefetch_related("choices").get(id=question_id)
        ...
        try:
            temperature += 0.1  # Gradualmente aumenta a temperatura
            self.retry(countdown=10, exc=exc, kwargs={"temperature": temperature})
        except MaxRetriesExceededError:
            raise Exception(f"Max retries exceeded for question {question_id}")
...
def handle_multiple_choice(question, temperature):
    ...
    # Se falhar ou vier vazio, não salva
    if answer_text:
...
def generate_multiple_choice_answer(
    ...
    """
    Gera a resposta para uma questão de múltipla escolha,
    valida e, se falhar, não faz retry com outro modelo.
    """
    ...
    # Delatexificar
    if question.subject in ["Matemática", "Química", "Física"]:
        answer_text_delatexified = LatexNodes2Text().latex_to_text(answer_text or "")
    else:
        answer_text_delatexified = answer_text
    if not answer_text_delatexified:
        logger.warning(f"Empty response from GPT for question {question.id}")
        return None

    # Valida a resolução passando a imagem diretamente
    validation_response = validate_resolution(
        question.text,
        answer_text_delatexified,
        choices,
    ...
    # Se vier "não", aborta e não salva resposta
    if validation_response and "não" in validation_response.lower():
        logger.warning(
            f"Validation suggests the answer might be wrong. No retry for question {question.id}"
        )
        return None
...
def generate_discursive_answer(question, temperature=0.1):
    """
    Gera a resposta para questão discursiva,
    valida e, se falhar, não faz retry com outro modelo.
    """
    image_url = question.image.url if question.image else ""
...
def generate_answer_text_for_multiple_choice_question(
    ...
    """
    Gera a resposta inicial para questão de múltipla escolha,
    usando 'o3-mini' ou 'gpt-4o' conforme a matéria, e passa a imagem diretamente.
    """
    global total_tokens
    ...
    user_message = (
        f"Enunciado:\n{question_text}\n\n"
        f"Alternativas:\n{chr(10).join(choices)}\n\n"
        f"Alternativa correta: {correct_choice}"
    )
    ...
    logger.info(
        f"Iniciando chamada para gerar resposta (Múltipla Escolha) - Questão: {question_text[:20]}..."
    )

    # Se a matéria for Mat/Química/Física -> "o3-mini" com get_completion_parsed (formato SimpleAnswer)
    if subject in ["Matemática", "Química", "Física"]:
        response = openai_utils.get_completion_parsed(
            model="o3-mini",
            messages=messages,
            response_format=SimpleAnswer,  # retorna objeto com campo "answer"
            reasoning_effort="high",
            timeout=60,
        )
        ai_response = response.content.answer
    else:
        # Caso contrário, usa gpt-4o normalmente
        response = openai_utils.get_completion(
    ...
    logger.info(
        f"GPT answer generation successful for question (Múltipla Escolha) {question_text[:20]}..."
    )
    return ai_response
...
def generate_answer_text_for_discursive_question(
    ...
    """
    Gera a resposta inicial para questão discursiva,
    usando 'o3-mini' ou 'gpt-4o' conforme a matéria, e passa a imagem diretamente.
    """
    global total_tokens
    ...
    user_message = (
        f"Texto extraído: {extra_embedding_text}\n\nEnunciado:\n{question_text}"
    )
    ...
    logger.info(
        f"Iniciando chamada para gerar resposta (Discursiva) - Questão: {question_text[:20]}..."
    )
    ...
    logger.info(
        f"GPT answer generation successful for question (Discursiva) {question_text[:20]}..."
    )
    return ai_response
...
def validate_resolution(
    ...
    """
    Valida a resolução chamando o modelo adequado:
    'o3-mini' para ['Matemática','Química','Física'] ou 'gpt-4o' caso contrário.
    Passa a imagem diretamente na mensagem e usa system_message específico
    conforme seja questão de múltipla escolha ou discursiva.
    """
    global total_tokens

    try:
        # Definir mensagem de sistema e de usuário
        ...
        # Chama "o3-mini" se a matéria estiver em [Matemática, Química, Física], senão "gpt-4o"
        if subject in ["Matemática", "Química", "Física"]:
            response = openai_utils.get_completion_parsed(
                model="o3-mini",
                messages=messages,
...
@shared_task(bind=True, max_retries=5)
def find_errors(self, question_id: int, temperature: float = 0):
    try:
        question = Question.objects.prefetch_related("choices").get(id=question_id)

        # Skip if question is already inactive
        if not question.is_active:
            logger.info(f"Skipping inactive question: {question_id}")
            return

        correct_probability = validate_question_and_choices(question, temperature)

        if correct_probability <= 75:
            logger.warning(
                f"Question {question_id} has low correct probability ({correct_probability}%). Deactivating."
            )
        ...
        try:
            temperature += 0.1  # Gradually increase temperature on retries
            self.retry(countdown=10, exc=exc, kwargs={"temperature": temperature})
        except MaxRetriesExceededError:
            raise Exception(f"Max retries exceeded for question {question_id}")
    except Exception as e:
...
def validate_question_and_choices(question, temperature: float = 0) -> int:
    """
    Validates a question using GPT-4o to check for errors or missing information.
    Returns an integer from 0 to 100 representing the probability of the question being error-free.
    """
    global total_tokens

    # Prepare question data
    question_text = question.text
    extra_text = question.extra_embedding_text or ""
    image_url = question.image.url if question.image else ""

    # Prepare choices if they exist
    choices_text = ""
    if question.choices.exists():
        choices = [choice.text for choice in question.choices.all()]
        choices_text = "\n".join(
    ...
    # User message
    user_message = f"Enunciado:{extra_text}\n\n{question_text}"
    if choices_text:
        user_message += f"\n\nAlternativas:\n{choices_text}"

    # Prepare messages for API call
    messages = [
        {"role": "system", "content": SYSTEM_MESSAGE_FIND_ERRORS},
        {
            "role": "user",
            "content": (
                [
    ...
    logger.info(f"Validating question {question.id}: {question_text[:50]}...")

    # Call the API
    response = openai_utils.get_completion("gpt-4o", temperature, messages, False, 30)
    ...
    try:
        ...
        error_probability = int(ai_response)

        # Ensure the value is within the valid range
        if error_probability < 0:
            error_probability = 0
        elif error_probability > 100:
            error_probability = 100

        logger.info(f"Question {question.id} validation result: {error_probability}%")
        return error_probability

    except ValueError:
        # If conversion fails, log the error and return a default value
        logger.error(
            f"Invalid response from GPT for question {question.id}: {ai_response}"
        )
        return 50  # Default to 50% if we can't parse the response
...
def update_urls_from_orm(questions_and_answers: list[dict[str, Any]]) -> None:
    """
    If you try to grab urls directly from SQL, you'll get urls without the prefix.
    This function updates SQL results with full URLs for questions and their choices.

    Assumes that the input follows the format:
    [
        {
            "id": <question_id>,
            "image": '',
    ...
    question_ids = [q["id"] for q in questions_and_answers]

    # Fetch questions with their choices in a single query
    ...
    # Update questions with URL mappings
