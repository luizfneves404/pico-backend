...
async def new_generate_transcriptions(
    ...
    """
    Recebe uma lista de imagens **ou** PDFs, gera transcrições, particiona
    ...
    if not all(f.content_type in VALID_MIME_TYPES_FOR_TRANSCRIPTION for f in files):
        tipos = ", ".join([f.content_type for f in files])  #! Testar se isso funciona
        raise ValueError(
            f"Formatos enviados não suportados ({tipos}). "
            "Aceitamos apenas JPEG, PNG, JPG e PDF."
        )

    # Garante que o lote contenha **apenas** imagens OU **apenas** PDFs
    ...
    if images:
        # 2.1 upload paralelo para S3 / storage usando create_file
        try:
            logger.info(f"Flow {flow_id}: carregando {len(images)} imagens para S3")
            # Cria objetos File no banco e faz upload para storage
            file_objects = await asyncio.gather(
                *(create_file(db_session, img) for img in images)
            )
            # Faz flush para garantir que os Files estão salvos antes de obter URLs
            await db_session.flush()

            # Obtém as URLs dos arquivos para a transcrição
            urls = await asyncio.gather(
                *(file_obj.get_url() for file_obj in file_objects)
            )
        except Exception as e:
            logger.exception("Falha no upload das imagens")
            raise

        # 2.2 transcrição paralela via OpenAI
        ...
        tasks = [
            gemini_utils.transcribe_uploaded_pdf(pdf) for pdf in pdfs
        ]  # uma página = um UploadFile
        page_texts = await asyncio.gather(*tasks)
        transcripts = [page_text for page_text in page_texts]
    ...
    async with db_manager.session_with_transaction() as db_session:
        ...
        if images:
            ...
            try:
                ...
                # Obter URLs dos arquivos para transcrição
                urls = await asyncio.gather(
...
async def get_files_user_generated_questions(
    ...
    try:
        ...
        # Comentado temporariamente devido a problema no admin
        # query = (
        #     select(FlowTranscriptionBlock)
        ...
        # Criando blocos manualmente para teste - Revolução Francesa
        class MockTranscriptionBlock:
            def __init__(self, block_number, title, block_text):
                self.block_number = block_number
                self.title = title
                self.block_text = block_text

        transcription_blocks = [
            MockTranscriptionBlock(
                1,
                "Origens e Início da Revolução Francesa (1789)",
                "A Revolução Francesa, deflagrada em 1789, foi o resultado de uma teia de tensões acumuladas ao longo de décadas, nascida do choque entre uma sociedade estamental rígida e uma burguesia ascendente que clamava por mobilidade e representação. O Antigo Regime sustentava-se em privilégios fiscais do clero e da nobreza, enquanto o Terceiro Estado — cerca de noventa e oito por cento da população — arcava com tributos crescentes que financiavam guerras, luxo palaciano e uma máquina administrativa inchada. Simultaneamente, ideias iluministas difundiam visões de soberania popular, contrato social e igualdade civil, corroendo a legitimidade de Luís XVI. A profunda crise financeira, agravada pela ajuda dispendiosa à independência norte-americana e por más colheitas na década de 1780, levou o rei a convocar os Estados-Gerais, evento que ofereceu à opinião pública — alimentada por panfletos e jornais — um raro palco para questionar o poder absoluto. Quando o Terceiro Estado proclamou-se Assembleia Nacional em junho, reivindicando legislar em nome da nação, inaugurou-se um processo revolucionário que logo escaparia ao controle das elites. O Juramento do Jogo da Péla selou a determinação dos deputados, e a tomada da Bastilha, em 14 de julho, tornou-se símbolo da insurreição popular contra o despotismo. Nas províncias, camponeses atacaram castelos, queimaram registros de servidão e precipitaram o Grande Medo, pressionando a Assembleia a abolir os direitos feudais. Assim, em poucos meses, ruíram alicerces seculares de ordem social, abrindo caminho para experimentos políticos inéditos. Essa primeira fase deixou claro que, dali em diante, nenhuma autoridade governaria a França sem negociar com uma cidadania vigilante e armada.",
            ),
            MockTranscriptionBlock(
                2,
                "Fase Constitucional da Revolução (1789-1792)",
                'A etapa constitucional, transcorrida entre 1789 e 1792, procurou conciliar a tradição monárquica com os novos princípios de liberdade cívica e de soberania nacional. A Assembleia Nacional Constituinte aboliu privilégios, secularizou bens do clero e promulgou a Declaração dos Direitos do Homem e do Cidadão, cujo enunciado — "os homens nascem e permanecem livres e iguais em direitos" — virou estandarte universal. Contudo, a Constituição de 1791 instituiu sufrágio censitário e criou a figura do "rei dos franceses", mantendo a coroa como poder executivo submetido à lei. Essa solução de compromisso não pacificou o país: clérigos refratários recusaram-se a jurar lealdade civil, nobres emigrados tramavam invasões e as massas urbanas ressentiam-se da carestia persistente. O pulso político acelerou-se em Paris, onde clubes como Jacobinos, Cordeliers e Feuillants travavam embates acalorados, enquanto jornais revolucionários alimentavam a opinião de tavernas e praças. Nos bairros populares, os sans-culottes exigiam pão barato e punição aos "traidores do povo", lançando mão de manifestos, marchas e guilhotinas simbólicas. A tensão transbordou em 1791, quando a fuga mal-sucedida de Luís XVI para Varennes minou a confiança na monarquia constitucional. Simultaneamente, potências absolutistas — Áustria e Prússia — ameaçavam intervir para restaurar o rei, temendo o contágio revolucionário. Ao declarar guerra em abril de 1792, a recém-eleita Assembleia Legislativa procurou unificar a nação, mas acabou potencializando o patriotismo radical. A insurreição das Tulherias, em 10 de agosto, depôs Luís XVI e inaugurou a Convenção Nacional, convocada por sufrágio universal masculino. Nascia a república, agora intimamente ligada à promessa de igualdade cívica e à defesa armada de seus valores.',
            ),
            MockTranscriptionBlock(
                3,
                "Terror e Consolidação da República (1793-1799)",
                'O período do Terror, entre 1793 e 1794, conduzido pelos jacobinos e pelo Comitê de Salvação Pública, aprofundou a radicalização revolucionária ao estabelecer que "virtude e terror" deveriam caminhar juntos. Diante de invasões estrangeiras e rebeliões internas — Vendéia, federalistas do sul, levante girondino — a Convenção aprovou a Lei dos Suspeitos, acelerou julgamentos em tribunais revolucionários e requisitou excedentes alimentares, enquanto tabelava preços para conter a carestia. Estimativas apontam para cerca de quarenta mil execuções, entre elas Maria Antonieta, Lavoisier e Danton. Paradoxalmente, o governo articulou a primeira mobilização em massa, criando exércitos cidadãos que derrotaram coalizões europeias em Fleurus, ao mesmo tempo em que laicizou a educação, descristianizou calendários e padronizou pesos e medidas. Mas a dinâmica de purificação devorou seus próprios líderes: em 9 Termidor (27 de julho de 1794) Robespierre caiu na guilhotina, encerrando o Terror e abrindo caminho para uma reação moderada. A Constituição do Ano III (1795) criou o Diretório, um executivo colegiado sustentado por sufrágio censitário, tentativa de frear tanto o radicalismo popular quanto a restauração absolutista. Crises econômicas, corrupção e guerras incessantes minaram-lhe o prestígio, favorecendo o golpe de 18 Brumário (9 de novembro de 1799), no qual Napoleão Bonaparte depôs o Diretório e assumiu o poder como primeiro cônsul. Embora a Revolução cedesse lugar ao Consulado, seu legado — direitos universais, cidadania, nacionalismo — continuaria a inspirar movimentos do século XIX e além. No imaginário moderno, a epopeia revolucionária tornou-se referência de ruptura contra tiranias e injustiças.',
            ),
        ]
...
async def get_files_query_official_questions(
    ...
    try:
        ...
        # Criando títulos mockados para teste - Revolução Francesa
        block_titles = [
            "Origens e Início da Revolução Francesa (1789)",
            "Fase Constitucional da Revolução (1789-1792)",
            "Terror e Consolidação da República (1793-1799)",
