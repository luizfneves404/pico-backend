...
class NotificationBatch:
    def __init__(
        self,
        user_notification_tuples: list[tuple[int, T]],
    ):
...
async def prepare_cleaned_text_notification(
    ...
    await fcm_service.send_notifications(
        [
            fcm_service.NotificationData(
                ...
                title=f"Extração da redação {essay_topic_name}",
                body="Abra o app para pedir a correção da sua redação!",
            )
        ]
    )
...
async def prepare_essay_correction_notification(
    ...
    await fcm_service.send_notifications(
        [
            fcm_service.NotificationData(
                user_id=receiver_user_id,
                title=f"Correção da redação {essay_topic_name}",
                body="Abra o app para ver a correção da sua redação!",
            )
        ]
    )
    return notification_batch
